import { type ContentStatus, type PlatformVariantKind, type Prisma, prisma } from '@qyro/db';
...
export default async function contentPiecesRoutes(app: FastifyInstance) {
  ...
  app.get('/content-pieces', { preHandler: [app.requireUser, wm] }, async (req) => {
  ...
  // ── Detalle ────────────────────────────────────────────────────────────────
  app.get('/content-pieces/:id', { preHandler: [app.requireUser, wm] }, async (req, reply) => {
    ...
    if (!piece || piece.workspaceId !== req.workspace!.id) {
      reply.code(404);
  ...
  // ── Aprobar → APPROVED | SCHEDULED ────────────────────────────────────────
  app.post('/content-pieces/:id/approve', { preHandler: [app.requireUser, wm] }, async (req, reply) => {
    const { id } = z.object({ id: z.string().min(1) }).parse(req.params);
    const body = z.object({ qcChecklist: z.record(z.boolean()).optional() }).parse(req.body ?? {});
    const workspaceId = req.workspace!.id;

    const piece = await prisma.contentPiece.findUnique({ where: { id }, include: { variants: true } });
    if (!piece || piece.workspaceId !== workspaceId) {
      reply.code(404);
      return { error: 'NOT_FOUND' };
    }
    const allScheduled = piece.variants.length > 0 && piece.variants.every((v) => v.scheduledAt !== null);
    ...
    if (nextStatus === 'SCHEDULED') {
      ...
      app.log.info({ contentPieceId: id, workspaceId, ...r }, 'jobs de publicación encolados');
    }
  ...
  // ── Rechazar → REJECTED ────────────────────────────────────────────────────
  app.post('/content-pieces/:id/reject', { preHandler: [app.requireUser, wm] }, async (req, reply) => {
    const { id } = z.object({ id: z.string().min(1) }).parse(req.params);
    ...
    const updated = await prisma.contentPiece.update({ where: { id }, data: { status: 'REJECTED' } });
    await recordStatusChange({
      contentPieceId: id,
      workspaceId,
      fromStatus: piece.status,
  ...
  app.post('/content-pieces/:id/request-changes', { preHandler: [app.requireUser, wm] }, async (req, reply) => {
    const { id } = z.object({ id: z.string().min(1) }).parse(req.params);
  ...
  app.patch('/content-pieces/:id/variants/:variantId/schedule', { preHandler: [app.requireUser, wm] }, async (req, reply) => {
    const { id, variantId } = z
      .object({ id: z.string().min(1), variantId: z.string().min(1) })
      .parse(req.params);
    const body = z.object({ scheduledAt: z.string().datetime({ offset: true }).nullable() }).parse(req.body);
    const workspaceId = req.workspace!.id;
    ...
    if (!piece || piece.workspaceId !== workspaceId || piece.variants.length === 0) {
      reply.code(404);
      return { error: 'NOT_FOUND' };
    }
  ...
  app.post('/content-pieces/:id/duplicate', { preHandler: [app.requireUser, wm] }, async (req, reply) => {
    const { id } = z.object({ id: z.string().min(1) }).parse(req.params);
    const workspaceId = req.workspace!.id;
